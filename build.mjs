...
<body><a class="skip" href="#main">跳转到正文</a><header><div class="nav-wrap"><a class="brand" href="${prefix}index.html">${esc(site.name)} <span class="brand-english">${esc(site.englishName)}</span></a><nav aria-label="主导航">${[['home', '首页', 'index.html'], ['projects', '项目', 'projects/index.html'], ['blog', '博客', 'blog/index.html']].map(([id, label, url]) => `<a href="${prefix}${url}" ${active === id ? 'aria-current="page"' : ''}>${label}</a>`).join('')}<button class="theme" aria-label="切换深色模式" title="切换配色">◐</button></nav></div></header>
...
const intro = `<section class="intro intro-revised intro-live"><div class="intro-copy"><div class="intro-heading"><h1>${esc(site.name)} <span class="name-english">${esc(site.englishName)}</span></h1><p class="intro-motto">${esc(site.motto)}</p></div><div class="intro-details"><dl class="about-list"><div><dt>研究方向</dt><dd><span class="accent">${esc(site.research)}</span></dd></div><div><dt>教育经历</dt><dd class="education-list">${site.education.map(e => `<p><span class="education-period">${esc(e.period)}</span> <strong>${esc(e.school)}</strong> <span>${esc(e.degree).replace(" · ", " ")}</span></p>`).join("")}</dd></div>${site.experience?.length ? `<div><dt>实习经历</dt><dd>${site.experience.map(e => `<p><span>${esc(e.period)}</span> <strong>${esc(e.organization)}</strong> <span>${esc(e.role)}</span></p>`).join('')}</dd></div>` : ''}<div><dt>荣誉与奖励</dt><dd class="honor-groups">${site.honors.map(h => `<div class="honor-group"><span class="honor-category">${esc(h.category)}</span><ul class="honors-list">${h.items.map(item => `<li>${esc(item)}</li>`).join("")}</ul></div>`).join("")}</dd></div></dl></div></div><aside class="profile">${portrait}</aside><div class="intro-closing"><p class="site-purpose">${esc(site.purpose)}</p><div class="socials" aria-label="其他账号">${external(site.github, "GitHub", "profile-link")}<a href="mailto:${esc(site.email)}" class="profile-link">${esc(site.email)} <span aria-hidden="true">↗</span></a></div></div></section>`;
...
write('.nojekyll', '');
write('_redirects', '/optics-scholar-hub https://uestczhaoyan-gif.github.io/optics-scholar-hub/ 302\n/optics-scholar-hub/* https://uestczhaoyan-gif.github.io/optics-scholar-hub/:splat 302\n');
